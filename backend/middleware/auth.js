...
const authenticate = async (req, res, next) => {
  try {
    ...
    const result = await pool
      ...
      .query(`
        SELECT UserID, FullName, Email, Role, IsActive, IsVerified, AvatarURL, IsFPTStudent
        FROM Users
        WHERE UserID = @UserID
      `);
...
const optionalAuth = async (req, res, next) => {
  try {
    ...
    const result = await pool
      .request()
      .input('UserID', sql.Int, decoded.userId)
      .query(`SELECT UserID, FullName, Email, Role, IsActive, IsVerified, IsFPTStudent FROM Users WHERE UserID = @UserID`);
