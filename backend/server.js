  ...
  app.get("/api/debug/check-admin", async (req, res) => {
    try {
      ...
      if (result.recordset.length === 0) {
        return res.json({
          found: false,
          message: "Admin not found. Run: node scripts/seed-admin.js",
        });
      }
  ...
  app.get("/api/debug/reset-admin", async (req, res) => {
    try {
      ...
      await pool
        ...
        .query(
          "UPDATE Users SET PasswordHash=@Hash, IsActive=1, IsVerified=1, Role='Admin' WHERE Email=@Email",
        );
      res.json({
        success: true,
...
const startServer = async () => {
  await connectDB();
  app.listen(PORT, () =>
    console.log(`🚀 EMS Backend: http://localhost:${PORT}`),
  );
};
startServer();
module.exports = app;
