...
const emailLimiter = rateLimit({
  windowMs: 60 * 60 * 1000,
  max: 5,
  message: {success: false, message: "Quá nhiều yêu cầu gửi email."},
});
...
router.post(
  "/register",
  authLimiter,
  uploadOrgDocs.fields([{name: "documents", maxCount: 5}]),
  registerRules,
  validate,
  register,
);
...
router.get(
  "/admin/pending-speakers",
  authenticate,
  authorize("Admin"),
  getPendingSpeakers,
);
router.post(
  "/admin/organizers/:profileId/review",
  authenticate,
  authorize("Admin"),
  approveOrganizer,
);
router.post(
  "/admin/speakers/:speakerId/review",
  authenticate,
  authorize("Admin"),
  approveSpeaker,
);
